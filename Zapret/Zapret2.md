...
Проще: zapret2 лучше не «по скорости», а по гибкости и живучести.

В nfqws1 (обычный Zapret) логика обхода DPI была «запаяна» в C: чтобы что-то поменять, нужны знания C и пересборка. В zapret2 это вынесли в скрипты Lua: сам «двигатель» пакетный остался в C, а стратегии работают как конфигурация/скрипты. Поэтому срабатывание под новые блокировки можно быстро допиливать и тестировать, не переписывая ядро.

Появилась библиотека готовых Lua-подходов (TLS/QUIC/HTTP манипуляции, хелперы), которые можно расширять и комбинировать.

Отсюда главный плюс: кто умеет работать с пакетами, может создавать/шить свои методы обхода и делиться ими, а не ждать полной переработки релизов.

В итоге: одинаковый базовый функционал (распознавание протоколов, декодинг, фильтры и т.д.), но zapret2 легче адаптировать под меняющиеся методы DPI.

Коротко: zapret — «жёсткий и более статичный», zapret2 — «параметризуемый и масштабируемый» под эволюцию цензуры.
