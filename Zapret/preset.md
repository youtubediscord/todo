...
img:
---
### Zapret 2 GUI основан на технике пресетов
**Zapret2** — инструмент для обхода DPI (Deep Packet Inspection) — систем глубокого анализа трафика, которые используются для блокировки сайтов и сервисов. В России это ТСПУ (технические средства противодействия угрозам).
...
**Пресет** (*часто называемые конфигами*) — это txt файл с настройками программы, вместо использования GUI (*формат такой же как у `winws2.exe`, чтобы программы могли быстро считать и изменить настройки программы*) 
...
Далее следуют [[Базовые аргументы (режимы фильтрации WinDivert)|глобальные фильтры WinDivert]], на языке [[Zapret2]] они имеют направление (`-in`, `-out`). По умолчанию (как правило) хватает только `-out`.
