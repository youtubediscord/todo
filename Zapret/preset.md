...
**Пресет** (*часто называемые конфигами*) — это `.txt` файл(ы) с перечислением всех [[Zapret2 - основные флаги|флагов]] Zapret2, которые можно использовать как вместо так и вместе с GUI (*формат такой же как у `winws2.exe`, чтобы программы могли быстро считать и изменить настройки программы*). Они доступны в программе начиная с `Zapret2 v20.3`.
