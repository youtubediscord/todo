...
AmneziaWG (AWG) — модифицированный WireGuard с обфускацией трафика для обхода DPI-блокировок.
...
| QUIC пример: `<b 0xc7000000010>` | 11 hex-цифр = **нечётное** → ОШИБКА | `obf_bytes.go` требует чётное количество hex-цифр |
| Signature packets отправляются "перед каждым handshake" | Только перед **Init**, НЕ перед Response | SendHandshakeResponse() не содержит ipackets/junk |
