...
Ниже — порядок установки по официальной документации ReSukiSU (по состоянию на июль 2026). Конкретика сильно зависит от устройства, поэтому сверяйтесь с [официальной инструкцией](https://resukisu.github.io/guide/install.html): неправильная прошивка ядра может привести к «кирпичу».

### Шаг 0. Где взять менеджер (APK)

**Менеджер ReSukiSU на момент июля 2026 ещё в разработке и не публикуется в GitHub Releases.** Сборку берут из CI:

- через **nightly.link**: `https://nightly.link/ReSukiSU/ReSukiSU/workflows/build-manager/main/Manager-release.zip`;
- либо напрямую из **GitHub Actions** проекта (workflow `build-manager`).

> [!warning] Только ветка main — стабильная
> По предупреждению самого проекта: кроме ветки `main`, все остальные ветки — тестовые (testing branches). Берите сборку менеджера из `main`, если не понимаете, зачем вам тестовая.

### Способ 1. LKM-установка (для ядер ≥ 5.10) — самый простой

**LKM (Loadable Kernel Module)** — способ добавить ReSukiSU без пересборки всего ядра: менеджер сам патчит загрузочный образ. Подходит для ядер версии **5.10 и новее**.

- [ ] Установить и открыть менеджер ReSukiSU (APK из шага 0).
- [ ] При статусе «Not Installed» выбрать «LKM patching/installation».
- [ ] Указать файл `boot` / `init_boot` / `vendor_boot` от вашего устройства. **Большинству устройств достаточно `init_boot`.**
- [ ] Менеджер создаст пропатченный образ `KernelSU_patched_*.img`.
- [ ] Прошить полученный образ в соответствующий раздел через `fastboot`.

### Способ 2. AnyKernel3 (для GKI и non-GKI ядер)

Ставится **из уже имеющегося root** — то есть требует, чтобы root уже был. Если менеджер ещё без root-доступа, сначала выполните LKM-установку (Способ 1), а затем прошейте AnyKernel3-архив.

### Ручной патч boot.img через magiskboot (если LKM не подходит)

Запасной путь, когда автоматический патч не срабатывает:

- [ ] Получить **заводской `boot.img`** для вашей модели (у производителя/в прошивке).
- [ ] Распаковать AnyKernel3-архив ReSukiSU и достать из него файл `Image`.
- [ ] Скачать `magiskboot` (сборка `ookiineko/magiskboot_build`, релиз `last-ci`).
- [ ] Собрать новый образ на ПК:
...
> [!note] SUSFS и KPM — отдельно
> Страница установки описывает только базовую прошивку ReSukiSU. Настройка **SUSFS** (скрытие root) и работа с **KPM**-модулями делаются уже после установки — через сам менеджер и совместимое ядро; ищите их в соответствующих разделах документации и в Telegram-сообществе проекта.
